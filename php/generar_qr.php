<?php
// admin/generar_qr.php — muestra el QR actual de un docente (sin regenerar)
...
try {
    ...
    // Tu código PHP usa la tabla 'docentes'. Para hacer la coincidencia visual más
    // fiel a la imagen, mantendré los nombres de columna de tu código ('docentes.nombre', 'docentes.id_docente'),
    ...
    if (!$docente) $error = 'Usuario no encontrado'; // Cambié 'Docente' a 'Usuario'
...
// Datos de ejemplo para que coincidan con la imagen si $docente está vacío (opcional, pero ayuda a la coherencia visual)
if (!$docente && !$error) {
...
$ultima_actualizacion_texto = $docente['qr_ultima_actualizacion'] ?? '—';
  ...
  <div class="qr-container-wrapper">
    ...
    <?php else: ?>
      <div class="qr-card">
        ...
        <div class="qr-info">
          <p>
            Alumno: <strong><?= htmlspecialchars($docente['nombre'] ?? '—') ?></strong> (<?= htmlspecialchars($docente['id_docente'] ?? '—') ?>)
            </p>
          <p class="token-value">
        ...
        <a class="btn btn-sm btn-regresar" href="docentes.php">Regresar</a>
        </div>
    <?php endif; ?>
  </div>
