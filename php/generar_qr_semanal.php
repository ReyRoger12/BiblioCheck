...
$alumno = null; $error = null; $token = null;

try {
    // Valida alumno
    $stmt = $conexion->prepare("SELECT id, id_alumno, nombre FROM alumnos WHERE id=? LIMIT 1");
    $stmt->bind_param('i', $id); $stmt->execute();
    $res = $stmt->get_result(); $alumno = $res->fetch_assoc(); $stmt->close();
    if (!$alumno) { throw new Exception('alumno no encontrado'); }

    $token = nuevoToken();
    $stmt = $conexion->prepare("UPDATE alumnos SET qr_semanal=?, qr_ultima_actualizacion=NOW() WHERE id=?");
    $stmt->bind_param('si', $token, $id);
    $stmt->execute(); $stmt->close();

    // Re-obtener la fecha de actualizacion (aunque ya se conoce que es NOW())
    $stmt = $conexion->prepare("SELECT qr_ultima_actualizacion FROM alumnos WHERE id=? LIMIT 1");
    $stmt->bind_param('i', $id); $stmt->execute();
    $res = $stmt->get_result();
  ...
  <div class="qr-container-wrapper">
    <div class="qr-card">
      ...
      <?php if ($error): ?>
        ...
        <a class="btn btn-secondary btn-action mt-3" href="alumnos.php">Regresar</a>
      <?php else: ?>
        <div class="qr-info">
          <p><strong>alumno:</strong> <span style="color:#343a40"><?= htmlspecialchars($nombre_alumno) ?></span></p>
          <p><strong>ID:</strong> <span><?= htmlspecialchars($id_alumno) ?></span></p>
        ...
        <div class="mt-4 d-flex gap-3 justify-content-center">
          <a class="btn btn-primary btn-action" href="generar_qr.php?id=<?= (int)$alumno_id ?>">Ver QR Estático</a>
          <a class="btn btn-secondary btn-action" href="alumnos.php">Regresar a Lista</a>
        </div>
      <?php endif; ?>
    </div>
  </div>
