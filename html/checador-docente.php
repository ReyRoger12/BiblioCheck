<?php
/************************************************************
 * DocenteTrack - checador-docente.php (corregido y robusto)
 ...
 * - Solo una entrada abierta a la vez en cualquier curso.
 * - No permite salida sin entrada previa del mismo curso.
 * - Evita doble salida el mismo día para el curso.
 */
function registrarAsistencia(mysqli $conexion, int $docente_id, int $curso_id, string $tipo): array
{
    ...
    try {
        // 1) ¿Tiene alguna entrada sin salida en cualquier curso?
        $sql = "
            SELECT a1.id
            FROM asistencia a1
            WHERE a1.docente_id = ?
              ...
              AND NOT EXISTS (
                ...
                WHERE a2.docente_id = a1.docente_id
                  AND a2.curso_id   = a1.curso_id
        ...
        $stmt->bind_param('is', $docente_id, $fecha_actual);
        $stmt->execute();
        $stmt->store_result();
        ...
        if ($tipo === 'entrada' && $entrada_abierta_en_alguno) {
            throw new Exception('No puedes registrar entrada a otra materia sin haber registrado antes la salida de la materia anterior.');
        }

        // 2) Si es salida, debe existir entrada previa para ese curso SIN salida posterior
        if ($tipo === 'salida') {
            // 2.1 Entrada previa y abierta para el curso
            $sql = "
                SELECT a1.id
                FROM asistencia a1
                WHERE a1.docente_id = ?
            ...
            $stmt->bind_param('iis', $docente_id, $curso_id, $fecha_actual);
            $stmt->execute();
            $stmt->store_result();
            $tiene_entrada_abierta_para_curso = $stmt->num_rows > 0;
            $stmt->free_result();
            $stmt->close();

            if (!$tiene_entrada_abierta_para_curso) {
                throw new Exception('No puedes registrar una salida sin haber registrado primero una entrada para este curso.');
            }

            // 2.2 Evitar doble salida hoy para el curso
            $sql = "
                SELECT a1.id
                FROM asistencia a1
                WHERE a1.docente_id = ?
            ...
            if ($ya_tiene_salida) {
                throw new Exception('Ya tienes una salida registrada para este curso hoy.');
            }
        }

        // 3) Insertar el registro de asistencia
        $sql = "
            INSERT INTO asistencia (docente_id, curso_id, fecha, hora, tipo)
            VALUES (?, ?, ?, ?, ?)
        ";
        $stmt = $conexion->prepare($sql);
        if (!$stmt) throw new Exception('Error al preparar inserción de asistencia.');
        $stmt->bind_param('iisss', $docente_id, $curso_id, $fecha_actual, $hora_actual, $tipo);
        if (!$stmt->execute()) {
            throw new Exception('No se pudo registrar la asistencia.');
        }
        $stmt->close();

        // 4) Obtener información del docente y curso
        $sql = "
            SELECT d.nombre, d.id_docente, c.nombre AS curso
            FROM docentes d
        ...
        ";
        $stmt = $conexion->prepare($sql);
        if (!$stmt) throw new Exception('Error al preparar obtención de datos del docente/curso.');
        ...
        if (!$datos) {
            ...
            $datos = ['nombre' => 'N/D', 'id_docente' => 'N/D', 'curso' => 'N/D'];
        }
...
/**
 * Devuelve clases asignadas a un id_docente (puede ser alfanumérico).
 */
function obtenerClasesDocente(mysqli $conexion, string $id_docente): array
{
    $sql = "
    ...
    ";
    $stmt = $conexion->prepare($sql);
    if (!$stmt) throw new Exception('Error al preparar carga de clases del docente.');
...
/**
 * Verifica token QR semanal para un docente activo.
 */
function verificarTokenQR(mysqli $conexion, string $token): array
{
    ...
    if (!$row) {
        return ['success' => false, 'message' => 'Token QR no válido o docente inactivo'];
    }

    return ['success' => true, 'docente' => $row];
}

/**
 * Retorna las clases con estado de si puede registrar entrada/salida hoy.
 */
function obtenerClasesConEstado(mysqli $conexion, int $docente_id, string $id_docente): array
{
    $fecha_actual = date('Y-m-d');
    $clases       = obtenerClasesDocente($conexion, $id_docente);

    // ¿Hay alguna entrada abierta hoy en cualquier curso?
    $sql = "
        SELECT a1.curso_id
        FROM asistencia a1
        WHERE a1.docente_id = ?
    ...
    $curso_abierto          = $entrada_abierta_global ? (int)$res->fetch_assoc()['curso_id'] : null;
    $stmt->close();
    ...
    foreach ($clases as $clase) {
        ...
        // ¿Tiene entrada abierta en este curso?
        $sql = "
            SELECT a1.id
            FROM asistencia a1
            WHERE a1.docente_id = ?
        ...
        ";
        $stmt = $conexion->prepare($sql);
        if (!$stmt) throw new Exception('Error al preparar consulta entrada abierta por curso.');
        $stmt->bind_param('iis', $docente_id, $cid, $fecha_actual);
        $stmt->execute();
        $stmt->store_result();
        $entrada_abierta_en_curso = $stmt->num_rows > 0;
        $stmt->free_result();
        $stmt->close();

        // ¿Ya tiene salida registrada hoy para este curso?
        $sql = "
            SELECT a1.id
            FROM asistencia a1
            WHERE a1.docente_id = ?
        ...
        ";
        $stmt = $conexion->prepare($sql);
        if (!$stmt) throw new Exception('Error al preparar consulta salida por curso.');
        $stmt->bind_param('iis', $docente_id, $cid, $fecha_actual);
        $stmt->execute();
        $stmt->store_result();
        ...
        $clases_con_estado[] = [
            'id'                         => $cid,
            'nombre'                     => $clase['nombre'],
            // Puede entrar si NO hay ninguna entrada abierta global y NO tiene ya salida en ese curso
            'puede_entrar'               => (!$entrada_abierta_global && !$tiene_salida),
            // Puede salir si TIENE entrada abierta en ese curso y NO tiene salida aún
            'puede_salir'                => ($entrada_abierta_en_curso && !$tiene_salida),
            'tiene_entrada_sin_salida'   => $entrada_abierta_en_curso,
            'es_curso_pendiente'         => ($entrada_abierta_global && $curso_abierto === $cid),
        ];
    }
        ...
        switch ($action) {
            case 'verificar_qr': {
                ...
                if ($result['success'] ?? false) {
                    // Guardar en sesión al docente reconocido por QR
                    $_SESSION['docente_qr'] = $result['docente'];
                }
                json_response($result);
            }

            case 'obtener_clases': {
                if (empty($_SESSION['docente_qr'])) {
                    throw new Exception('Sesión de docente no encontrada.');
                }
                $docRow     = $_SESSION['docente_qr'];
                $docente_id = (int)$docRow['id'];
                $id_docente = (string)$docRow['id_docente'];

                $clases_con_estado = obtenerClasesConEstado($conexion, $docente_id, $id_docente);
                if (!$clases_con_estado) {
                    throw new Exception('El docente no tiene clases asignadas.');
                }
            ...
            case 'registrar_asistencia': {
                ...
                $curso_id = expect_post('curso_id', '');
                $tipo     = trim((string)expect_post('tipo', ''));
                if ($curso_id === '' || $tipo === '') {
                    throw new Exception('Datos incompletos.');
                }
                if (!ctype_digit((string)$curso_id)) {
                    throw new Exception('ID de curso inválido.');
                }
                $curso_id = (int)$curso_id;

                $resultado = registrarAsistencia($conexion, $docente_id, $curso_id, $tipo);

                // Consumimos el QR; obliga a volver a escanear para otra acción
                unset($_SESSION['docente_qr']);
...
<head>
  ...
  <style>
    body {
           background-image: url("https://localhost/DOCENTETRACK/assets/FONDOB.jpg"), 
    ...
    .curso-pendiente { background:#0d6efd; color:#fff; }
    .curso-pendiente .clase-estado { color:#e0e0e0; }
...
<body>
  <div class="checador-container">
    ...
    <?php if (!empty($mensaje)): ?>
      <div class="alert alert-<?= htmlspecialchars($mensaje['tipo']) ?> alert-dismissible fade show">
        ...
        <p><strong>Docente:</strong> <?= htmlspecialchars($mensaje['datos']['nombre']) ?></p>
        <p><strong>ID:</strong> <?= htmlspecialchars($mensaje['datos']['id_docente']) ?></p>
        <p><strong>Clase:</strong> <?= htmlspecialchars($mensaje['datos']['curso']) ?></p>
  ...
  <script>
    ...
    async function verificarTokenQR(token) {
      try {
        detenerScanner();
        const resp = await fetch('checador-docente.php', {
          method: 'POST',
        ...
        // Mostrar confirmación de docente
        document.getElementById('scan-overlay').innerHTML = `
          <div class="qr-confirmacion">
            <i class="fas fa-check-circle"></i>
            <h4 class="mb-3">QR válido</h4>
            <p class="mb-1"><strong>Alumno:</strong> ${data.docente.nombre}</p>
            <p><strong>ID:</strong> ${data.docente.id_docente}</p>
          </div>`;
        document.getElementById('mensaje-exito').style.display = 'block';

        // Cargar clases y habilitar botones
        const r2   = await fetch('checador-docente.php', {
          method: 'POST',
          headers: {'Content-Type':'application/x-www-form-urlencoded'},
    ...
    async function mostrarSelectorClases(tipo) {
      tipoRegistro = tipo;
      try {
        const r = await fetch('checador-docente.php', {
          method: 'POST',
          headers: {'Content-Type':'application/x-www-form-urlencoded'},
          body: new URLSearchParams({ action:'obtener_clases' })
        });
        ...
        // Si es salida y hay curso pendiente, lo preseleccionamos (y podemos auto-confirmar)
        if (tipo === 'salida') {
          const pendiente = filtradas.find(c => c.es_curso_pendiente);
          if (pendiente) {
            lista.innerHTML = `
              <div class="clase-item curso-pendiente">
    ...
    async function registrarAsistenciaDirecta(cursoId, tipo) {
      try {
        const r = await fetch('checador-docente.php', {
          method: 'POST',
          headers: {'Content-Type':'application/x-www-form-urlencoded'},
          body: new URLSearchParams({ action:'registrar_asistencia', curso_id:String(cursoId), tipo:String(tipo) })
        });
        const d = await r.json();
        if (!d.success) {
...
ID: ${d.datos.id_docente}
Puesto: ${d.datos.curso}
Hora: ${d.datos.hora}`);
