...
// 1. Obtener datos del alumno (Añadimos semestre, carrera y horario_json)
$stmt_doc = $conexion->prepare("
    SELECT nombre, id_alumno, puesto, numero_control, semestre, carrera, horario_json 
    FROM alumnos 
    WHERE id = ?
");
...
if (!empty($alumno['horario_json'])) {
    // true = decodificar como array asociativo
    $horario_parseado = json_decode($alumno['horario_json'], true); 
}
// ============================================================================
...
<title>Control de Horario - <?= htmlspecialchars($alumno['nombre']) ?></title>
<style>
...
<header class="header-tecnm" style="
    ...
    <img src="../assets/LOGO_SEP.png" style="
        position: absolute;
        left: 0;
        top: 50%;
    ...
    <img src="../assets/logoB.jpg" alt="Logo Centrado" style="
        width: 60px;
  ...
  <table class="info-table">
    <tr>
      ...
      <td style="width:110px;"><?= htmlspecialchars($alumno['semestre'] ?? '') ?></td>
    </tr>
    <tr>
      ...
      <td class="info-right"><?= htmlspecialchars($alumno['carrera'] ?? '') ?></td>
    </tr>
    <tr>
